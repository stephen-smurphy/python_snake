            ...
            for y in range(position[1] - radius, position[1] + radius + 1):
                if x >= 0 and x < Constants.UNITS and y >= 0 and y < Constants.UNITS:
                    ...
                    if random.randint(0, distance * 3) < radius:
                        self.getByPosition(x, y).update_status(
                            PavementBlockStatus.BLOOD
                        )
...
class PavementBlock(pygame.sprite.DirtySprite):
    ...
    def update_status(self, status):
        if (
            status == PavementBlockStatus.BLOOD
            and self.status == PavementBlockStatus.BLOOD
        ):
            return
        self.status = status
        self.dirty = 1
        ...
        if self.status == PavementBlockStatus.NONE:
            ...
            # self.refill = Constants.FPS * (
            #     2 + random.randint(-1, 1)
            ...
            rect_a = pygame.Rect(
                int((random.randint(0, 10) / 10.0) * self.unit),
                int((random.randint(0, 10) / 10.0) * self.unit),
                size_a,
                size_a,
            )
            ...
            pygame.draw.rect(
                self.image, (140 + random.randint(-50, 50), 20, 10), rect_a
            )
            pygame.draw.rect(
                self.image, (140 + random.randint(-50, 50), 20, 10), rect_b
            )
            pygame.draw.rect(
                self.image, (140 + random.randint(-50, 50), 20, 10), rect_c
            )
        elif self.status == PavementBlockStatus.GRASS:
            # color_base = (110+random.randint(-50, 50),
            #               220+random.randint(-50, 30), 35)
            # self.image.fill(color_base)
